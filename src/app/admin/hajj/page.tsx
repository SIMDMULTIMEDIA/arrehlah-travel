...
export default async function HajjBookingsPage({
  searchParams,
}: {
  ...
  try {
    ...
    hajjBookings = fetched;
    totalBookings = count;
  } catch (error) {
    console.error("Database error fetching hajj bookings:", error);
  }
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-[var(--color-brand-navy)] flex items-center gap-3">
            <MapPin className="w-6 h-6" /> Hajj Operations
          </h1>
          <p className="text-sm text-slate-500">Manage Hajj applications, packages, and logistics.</p>
        </div>
      </div>
        ...
        <div className="p-4 border-b bg-slate-50 flex gap-4">
          <form className="flex-1 max-w-xl flex flex-col sm:flex-row gap-2">
            ...
            {(searchParams.q || searchParams.status) && (
              <Link href="/admin/hajj" className="px-4 py-2 bg-slate-200 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-300 whitespace-nowrap text-center">
                Clear
              </Link>
            )}
          </form>
          ...
          <table className="w-full text-left text-sm text-slate-600">
            <thead className="bg-slate-50 text-xs uppercase font-semibold text-slate-500 border-b">
              <tr>
                ...
                <th className="px-6 py-4">Hajj Year</th>
                <th className="px-6 py-4 text-center">Status</th>
                <th className="px-6 py-4 text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y">
              {hajjBookings.length === 0 ? (
                <tr>
        ...
        {totalPages > 1 && (
          <div className="px-6 py-4 border-t flex justify-between items-center bg-slate-50">
            ...
            <div className="flex gap-2">
              {page > 1 && (
                <Link href={`/admin/hajj?page=${page - 1}${searchParams.q ? `&q=${searchParams.q}` : ''}${searchParams.status ? `&status=${searchParams.status}` : ''}`} className="px-3 py-1 border rounded bg-white text-sm hover:bg-slate-50">
                  Previous
                </Link>
              )}
              {page < totalPages && (
