...
import { Eye, Search, Map } from "lucide-react";
...
export default async function UmrahBookingsPage({
  ...
  const searchFilter = searchParams.q ? {
    OR: [
      { packageType: { contains: searchParams.q, mode: "insensitive" as const } },
  ...
  let umrahBookings: any[] = [];
  let totalBookings = 0;

  try {
    const [fetched, count] = await Promise.all([
      prisma.umrahBooking.findMany({
        where: {
          ...statusFilter,
          ...searchFilter,
        },
    ...
    umrahBookings = fetched;
    totalBookings = count;
  } catch (error) {
    console.error("Database error fetching umrah bookings:", error);
  }
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-[var(--color-brand-navy)] flex items-center gap-3">
            <Map className="w-6 h-6" /> Umrah Operations
          </h1>
          <p className="text-sm text-slate-500">Manage Umrah packages, departure groups, and pilgrims.</p>
        </div>
      </div>
        ...
        <div className="p-4 border-b bg-slate-50 flex gap-4">
          <form className="flex-1 max-w-xl flex flex-col sm:flex-row gap-2">
            ...
            {(searchParams.q || searchParams.status) && (
              <Link href="/admin/umrah" className="px-4 py-2 bg-slate-200 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-300 whitespace-nowrap text-center">
                Clear
              </Link>
            )}
          </form>
                ...
                umrahBookings.map((booking) => (
                  <tr key={booking.id} className="hover:bg-slate-50">
                    <td className="px-6 py-4 font-medium text-[var(--color-brand-navy)]">
                      {booking.bookingItem.booking.reference}
                    </td>
                    <td className="px-6 py-4">
                      <div className="font-medium text-slate-900">
                        {booking.bookingItem.booking.user.firstName} {booking.bookingItem.booking.user.lastName}
                      </div>
                      <div className="text-xs text-slate-500">{booking.bookingItem.booking.user.email}</div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-center text-xs font-medium">
                      <span className={`inline-block px-2 py-1 rounded bg-slate-100 uppercase ${
                        booking.status === 'CONFIRMED' || booking.status === 'DEPARTED' ? 'bg-green-100 text-green-800' :
                    ...
                    <td className="px-6 py-4 text-right">
                      <Link
                        href={`/admin/bookings/${booking.bookingItem.booking.id}`}
        ...
        {totalPages > 1 && (
          <div className="px-6 py-4 border-t flex justify-between items-center bg-slate-50">
            <span className="text-sm text-slate-500">
              Showing {skip + 1} to {Math.min(skip + limit, totalBookings)} of {totalBookings} bookings
            </span>
            <div className="flex gap-2">
              {page > 1 && (
                <Link href={`/admin/umrah?page=${page - 1}${searchParams.q ? `&q=${searchParams.q}` : ''}${searchParams.status ? `&status=${searchParams.status}` : ''}`} className="px-3 py-1 border rounded bg-white text-sm hover:bg-slate-50">
                  Previous
                </Link>
              )}
              {page < totalPages && (
