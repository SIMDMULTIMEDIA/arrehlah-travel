...
import { submitUmrahRegistration } from "@/app/actions/umrah";
...
const formSchema = z.object({
  ...
  departureDate: z.string().min(1, "Please select a preferred departure date"),
  notes: z.string().optional(),
});

type FormValues = z.infer<typeof formSchema>;

interface UmrahRegistrationFormProps {
  initialPackage?: string;
}

export default function UmrahRegistrationForm({ initialPackage }: UmrahRegistrationFormProps) {
  ...
  const {
    ...
    defaultValues: {
      packageType: initialPackage || "Standard Package",
    },
  });

  useEffect(() => {
    if (initialPackage) {
      setValue("packageType", initialPackage);
    }
  ...
  const onSubmit = (data: FormValues) => {
    ...
    startTransition(async () => {
      ...
      const result = await submitUmrahRegistration(formData);

      if (result.success) {
        setSubmitStatus({
          success: true,
          message: "Registration successful! Our Umrah consultants will contact you shortly to confirm your booking.",
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 bg-white p-6 md:p-8 rounded-xl shadow-sm border">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium text-slate-700">Intended Departure Date <span className="text-red-500">*</span></label>
          <input
            type="date"
      ...
      <div className="space-y-2">
        ...
        <textarea
          ...
          rows={4}
          className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-[var(--color-brand-navy)] outline-none transition"
          placeholder="Any specific requests or requirements? (e.g. wheelchair assistance, connecting flights)"
        />
      </div>
